...
def resize_stationary_field(field, config):
    '''Extends the domain of the solution from $0 \leq x \leq d/2$ to $-d \leq x \leq d$ using its symmetry.
    
    Parameters
    ----------
    field : np.ndarray of floats
        The values of the field $u(t,x,z)$ at each x,z for $0 \leq x \leq d/2$. The array is of shape (N_x, N_z).
    config : TalbotConfig
        The class containing all the parameters of the simulation.
    ...
    resized_field : np.ndarray of floats
        The values of the field $u(t,x,z)$ at each x,z with for $-d \leq x \leq d$. Its shape is (4 N_x, N_z).
    '''
    # We allocate the new array
    resized_field = np.empty([4 * config.N_x, config.N_z])
